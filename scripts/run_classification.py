...
"""
Based on Transformers: examples/pytorch/text-classification/run_classification.py
https://github.com/huggingface/transformers/commit/aa1b09c5d1ab951bfd39778126a8a9a5cdf10ffc
TODO:
...
class DataTrainingArguments:
    ...
    zero_input: bool = field(
        default=False,
        metadata={
            "help": "Make all inputs empty string. For PVI."
        },
    )
    ...
    train_file: Optional[str] = field(
        default=None, metadata={"help": "A csv or a json/jsonl file containing the training data."}
    )
    validation_file: Optional[str] = field(
        default=None, metadata={"help": "A csv or a json/jsonl file containing the validation data."}
    )
    test_file: Optional[str] = field(default=None, metadata={"help": "A csv or a json/jsonl file containing the test data."})
    ...
    def __post_init__(self):
        if self.dataset_name is None:
            ...
            assert train_extension in ["csv", "json", "jsonl"], "`train_file` should be a csv or a json/ filejsonl."
            validation_extension = self.validation_file.split(".")[-1]
            assert (
                validation_extension == train_extension
            ), "`validation_file` should have the same extension (csv or json/jsonl) as `train_file`."
...
def main():
    ...
    def preprocess_function(examples):
        if data_args.zero_input: # zero input
            zero_inputs = [""] * len(examples["label"])
            examples["sentence"] = zero_inputs
            # examples.add_column('sentence', zero_inputs)
        elif data_args.text_column_names is not None:
