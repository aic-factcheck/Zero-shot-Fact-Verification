...
def config():
    ...
    PVI_ROOT = Path(QACG_ROOT, NLI_DIR, "pvi_fever_size_calibrated")

    MODEL_ROOT = "/home/drchajan/devel/python/FC/Zero-shot-Fact-Verification/experiments/nli/"
    MODEL_NAME = Path(MODEL_ROOT, "deepset/xlm-roberta-large-squad2_cs_CZ-20230801_fever_size_lr1e-6/checkpoint-98272_calibrated")
    NULL_MODEL_NAME = Path(MODEL_ROOT, "deepset/xlm-roberta-large-squad2_cs_CZ-20230801_fever_size_lr1e-6_zero_input/checkpoint-32_calibrated")
    MAX_LENGTH = 512

    DATESTR = datetime.now().strftime("%y%m%d_%H%M%S")
    NOTE = f"Computes PVI on Wiki-CS test split and save its extended version. QACG-CS model. QACG-CS model trained on FEVER-size version data."
