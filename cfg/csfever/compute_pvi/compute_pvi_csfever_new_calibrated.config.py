...
def config():
    ...
    NULL_MODEL_NAME = Path(MODEL_ROOT, "deepset/xlm-roberta-large-squad2_cs_CZ_lr1e-6_zero_input/checkpoint-32_calibrated")
    MAX_LENGTH = 512

    DATESTR = datetime.now().strftime("%y%m%d_%H%M%S")
    NOTE = f"Computes PVI on LREV CsFEVER-NLI test split and save its extended version. CsFEVER-NLI model (newer than LREV)."
