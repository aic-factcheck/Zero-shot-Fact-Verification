...
def config():
    ...
    PVI_ROOT = Path(NLI_ROOT, "pvi_calibrated")
    ...
    NULL_MODEL_NAME = Path(MODEL_ROOT, "deepset/xlm-roberta-large-squad2_en_US_lr1e-6_zero_input/checkpoint-256_calibrated")
    MAX_LENGTH = 512

    DATESTR = datetime.now().strftime("%y%m%d_%H%M%S")
    NOTE = f"Computes PVI on LREV EnFEVER-NLI test split and save its extended version. EnFEVER-NLI model (newer than LREV)."
